...
def ddet_log_guess(L: int, l1: int, l2: int,
             signs: tuple[bool, bool, bool] = (False, False, False)) -> mp.mpf:
    """
    Returns log(Ddet) where
      Ddet = |det(D)|^2 / 64^L
    ...
    # same piecewise map as B, applied to column index n
    mapped_n = np.empty_like(n)
    mapped_n2 = np.empty_like(n)
    if l1 > 0:
        mapped_n[:l1] = half + l1 + 1 - n[:l1]
        mapped_n2[:l1] = half + l1 + 1 - 2*n[:l1]
    if l2 > 0:
        mapped_n[l1:l1 + l2] = half + 2 * l1 + l2 + 1 - n[l1:l1 + l2]
        mapped_n2[l1:l1 + l2] = half + 2 * l1 + l2 + 1 - 2*n[l1:l1 + l2]
    if l1 + l2 < half:
        mapped_n[l1 + l2:] = L + l1 + l2 + 1 - n[l1 + l2:]
        mapped_n2[l1 + l2:] = L + l1 + l2 + 1 - 2*n[l1 + l2:]
    ...
    twopi_i_over_L = 2j * np.pi / L
    pi_i_over_L= 1j *np.pi/L
    row_half = m - 0.5   # half-integer shift: (m - 1/2)

    phase1 = twopi_i_over_L * (row_half[:, None] * n[None, :])
    phase2 = twopi_i_over_L * (row_half[:, None] * mapped_n[None, :])
    phase3 = pi_i_over_L*(mapped_n2[None,:])
    D = np.exp(phase1) + sign_vec[None, :] * np.exp(phase2)*np.exp(phase3)

    _, logabsdet = np.linalg.slogdet(D)
    return mp.mpf(2.0 * logabsdet) - mp.mpf(L) * _LOG64_MP


def ddet_log_guess2(L: int, l1: int, l2: int,
                    signs: tuple[bool, bool, bool] = (False, False, False)) -> mp.mpf:
    """
    Alternative D-matrix with geometric factors z_n * c_n on the second term.

    D[m,n] = exp(2πi(m-1/2)n/L) + s_j * z_n * c_n * exp(2πi(m-1/2)mapped(n)/L)

    where:
      - z_n = exp(2πi·n/L) (boundary point)
      - c_n = 1/sqrt(q_segment) where q depends on segment:
          q1 = -exp(2πi·l1/L)
          q2 = -exp(2πi·(l1+l2)/L)
          q3 = +exp(2πi·(l1+l2)/L)  (note: positive)
      - s_j ∈ {-1, +1} per segment (False -> -1, True -> +1)
    """
    if L % 2 != 0:
        raise ValueError("L must be even")
    half = L // 2
    ...
    m = np.arange(1, half + 1, dtype=np.float64)
    n = np.arange(1, half + 1, dtype=np.float64)
    ...
    # Boundary points z_n = exp(2πi·n/L)
    z = np.exp(2j * np.pi * n / L)

    # Segment-specific q factors
    q1 = -np.exp(2j * np.pi * l1 / L)
    q2 = -np.exp(2j * np.pi * (l1 + l2) / L)
    q3 = np.exp(2j * np.pi * (l1 + l2) / L)  # positive

    qs = np.empty(half, dtype=np.complex128)
    if l1 > 0:
        qs[:l1] = q1
    if l2 > 0:
        qs[l1:l1 + l2] = q2
    ...
    # Sign vector (real ±1)
    sign_vec = np.empty(half, dtype=np.float64)
    if l1 > 0:
        sign_vec[:l1] = 1 if signs[0] else -1
    if l2 > 0:
        sign_vec[l1:l1 + l2] = 1 if signs[1] else -1
    if l1 + l2 < half:
        sign_vec[l1 + l2:] = 1 if signs[2] else -1
    ...
    phase1 = twopi_i_over_L * (row_half[:, None] * n[None, :])
    phase2 = twopi_i_over_L * (row_half[:, None] * mapped_n[None, :])
    D = np.exp(phase1) + (sign_vec[None, :] * z[None, :] * c[None, :]) * np.exp(phase2)
