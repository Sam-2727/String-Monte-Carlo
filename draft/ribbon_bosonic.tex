...
\email{yuchen\_wang@fas.harvard.edu, xiyin@fas.harvard.edu, bmazel@g.harvard.edu\\ (please enter your email by yourself)}
...
\ie
Z(R;L;\ell_a) = 2\pi R\sum_{n\in \mathbb{Z}^{2g}}\int_{k\in \mathcal{K}'} \prod_{k\in \mathcal{K}}dX(k) \exp\left[-\sum_{k,\ell\in\mathcal{K}'}\mathbf{A}'_{k\ell}X(k)X(\ell)+2\sum_{k\in \mathcal{K}'}\mathbf{v}'_k X(k) +\mathbf{C}\right],
\fe
...
Correlators of the fermionic $(b_\lambda, c_{1-\lambda})$ system on higher-genus Riemann surfaces can be computed from the holomorphic data computed in section \ref{holodata} using the formula \cite{Verlinde:1986kw}
\ie
...
\fe 
where $n$ and $m$ must saturate the ghost number anomaly $n-m=(2\lambda-1)(g-1)$. The definitions of the chiral boson partition function $Z_{\text{chiral}}$, the Abel-Jacobi map $\zeta(z)$, the Riemann constant vector $\Delta$, the prime form $E(z,w)$, and the function $\sigma(z)$ will be discussed below in detail, together with the algorithm we use to compute them from the holomorphic 1-forms $\omega_i$ and the period matrix $\Omega$.
...
Notably, the quantity $Z_{\text{chiral}}$ is dependent on the Weyl frame and subject to gravitational anomaly due to the unbalanced central charge $(1,0)$. To specify $Z_{\text{chiral}}$ one must 

\subsubsection*{The Abel-Jacobi map}

\subsubsection*{Spin structures and the prime form}
