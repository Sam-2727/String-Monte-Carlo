...
def defaultDetCases : List (Nat × Nat × Nat) :=
  [(160, 40, 30), (320, 80, 60), (640, 160, 120), (960, 240, 180)]

structure DetBenchConfig where
  cases : List CylCase := []
  repeats : Nat := 5
  number : Nat := 1
  warmup : Nat := 1
...
partial def parseArgs : DetBenchConfig → List String → Except String DetBenchConfig
  | cfg, [] =>
      if cfg.cases.isEmpty then do
        let cases ← defaultDetCases.mapM (fun (L, l1, l2) =>
          match CylCase.mk? L l1 l2 with
          | .ok c => .ok c
          | .error e => .error e)
...
def runConfig (cfg : DetBenchConfig) : IO UInt32 := do
  if cfg.repeats == 0 || cfg.number == 0 then
    IO.eprintln "repeat and number must be positive."
    return 1
  let mut results : Array DetCaseBenchmark := #[]
  for c in cfg.cases do
    let r ← benchmarkDetCase c cfg.repeats cfg.number cfg.warmup cfg.targetMs
    results := results.push r
    if !cfg.json then
      printDetCaseBenchmark r
  if cfg.json then
