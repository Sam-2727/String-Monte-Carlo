...
\subsubsection*{Genus 1}

The wavefunctional of the identity operator in the $bc$ system satisfies
\ie
b_{m\geq -1} \Psi_1[c]=0,\quad c_{m\geq 2} \Psi_1[c]=0
\fe
since $b$ is the conjugate momentum of $c$, the path integral over $(b,c)$ is actually a phase space path integral instead of a configuration space one. One can choose a polarization of the phase space such that $c$ is the canonical coordinates, and the wavefunctional $\Psi[c]$ is labelled purely by a configuration of $c$ in the Schr\"odinger picture. Or equivalently, one can label $\Psi[c_m]$ as a function of the modes $c_m$, and $b_m$ acts on $\Psi[c_m]$ as the operator
\ie
\hat{b}_m = \frac{\partial}{\partial c_{-m}}
\fe
where we adopt the convention where all derivatives acts from the left. One can then find
\ie
\Psi_1[c]= \prod_{m=2}^\infty c_m,\quad \Psi_c[c]=\prod_{m=1}^\infty c_m
\fe

At genus 1, the correlator we are after is $\langle b\tilde{b}c\tilde{c}\rangle$. For the holomorphic part, one can simply consider $\langle b(z)c(0)\rangle$, where we put the $b$-ghost on a point with $|z|=1$ in the Strebel disk frame. In the continuum case, this correlator (in the disk frame) is given by
\ie
\langle b(z)c(0)\rangle &= \int [\mathcal{D}c] \delta_\text{id}[c] \hat{b}(z) \Psi_c[c]  = \int [\mathcal{D}c]\delta_\text{id}[c] \sum_{m} \frac{1}{z^{m+2}} \frac{\partial}{\partial c_{-m}} \Psi_c[c] \\
&=\int [\mathcal{D}c]\delta_\text{id}[c]\sum_{m} z^{m-2} (-1)^{m-1}\prod_{n=1, n\neq m}^\infty c_n
\label{continuumbc}
\fe
where $\delta_\text{id}$ is the identification map. Unlike the case of the free or compact boson, $c(z)$ has nontrivial conformal weight $-1$, hence the quantity that's identified should be the formal quantity $c(z)(dz)^{-1}$ instead of only $c(z)$. On each edge $a$, the identification map reads $\sigma+\sigma'=\theta_a$, namely $zz'=q_a$ where $q_a=e^{i\theta_a}$ is a phase, hence
\ie
\delta_\text{id}[c]=\prod_{\text{edges }a}\ \prod_{\sigma \in S_{i_1}(a)}\delta\left(c(\sigma)+e^{-2i\sigma+i\theta_a} c(\theta_a-\sigma)\right)
\fe

Now we want to discretize \eqref{continuumbc}. As usual, we discretize the disk into $L$ sites, and the delta function imposes the identification
\ie
c(k') = - e^{\frac{2\pi i}{L}(k'-k)}c(k)
\fe
and the mode $c_m$ admits the discrete Fourier transformation as $c_m = \frac{1}{L}\sum_{k=1}^L c(k)e^{\frac{2\pi i}{L}(m-1)k}$. The correlator $\langle b(z)c(0)\rangle$ is then given by
\ie
\langle b(z)c(0)\rangle &= \int \prod_{k=1}^{L/2}dc(k) \sum_m z^{m-2}(-1)^{m-1} \prod_{n=1, n\neq m}^{L/2+1}\sum_{k=1}^{L/2} c(k)e^{-\frac{2\pi i}{L}k}\left(e^{\frac{2\pi i}{L}mk}-e^{\frac{2\pi i}{L}mk'}\right)\\
&=\sum_m z^{m-2} (-1)^{m-1} \det B^{(m)}
\fe
where the $B^{(m)}$ is defined by stripping off the $m$-th row of the following $(\frac{L}{2}+1)\times \frac{L}{2}$ matrix:
\ie
B_{mk}= e^{-\frac{2\pi i}{L}k}\left(e^{\frac{2\pi i}{L}mk}-e^{\frac{2\pi i}{L}mk'}\right)
\fe
...
where
\begin{enumerate}
\item[$\bullet$] $Z_1$ is the chiral boson partition function, namely the contribution of the Fock space generated by holomorphic modes $\alpha_{-n}$ to the partition function, which is dependent on the Weyl frame and subject to gravitational anomaly due to the unbalanced central charge. One should avoid using $Z_1$ and use $|Z_1|^2$ instead to cancel the gravitational anomaly. 
...
\ie
Z_\text{bos}(\Omega) = V_X\int \frac{d^gk}{(2\pi)^g} e^{-\pi k^I k^J\mathrm{Im}(\Omega_{IJ})}|Z_1|^2 \propto \left[\det \mathrm{Im}(\Omega_{IJ})\right]^{-1/2}|Z_1|^2
\fe
We can choose our renormalization scheme such that
\ie
|Z_1|^2 = \left[\det \mathrm{Im}(\Omega_{IJ})\right]^{\frac{1}{2}} [(\det A')^{-\frac{1}{2}}]_\text{renormalized}
\fe
...
\ie
\left|\langle b(z_1)b(z_2)b(z_3) \rangle\right|^2 |Z_1(\Omega)|^{52} \propto |\det(S_i(z_j))|^2|\Phi(\Omega)|^2
\fe
...
\fe
where $\chi_{10}$ is the Igusa cusp form. Since the quantity $\left|\langle b(z_1)b(z_2)b(z_3) \rangle\right|^2 |Z_1|^{52}$ is free from Weyl anomaly, one can check that
\ie
...
\fe
which should hold for general moduli $\Omega_1, \Omega_2$.
