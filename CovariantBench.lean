...
def defaultCases : List (Nat × Nat × Nat) :=
  [(12, 2, 2), (16, 4, 4), (20, 4, 4), (40, 10, 8)]
...
partial def parseArgs : BenchConfig → List String → Except String BenchConfig
  | cfg, [] =>
      if cfg.cases.isEmpty then do
        let cases ← defaultCases.mapM (fun (L, l1, l2) =>
          match CylCase.mk? L l1 l2 with
          | .ok c => .ok c
          | .error e => .error e)
...
def runConfig (cfg : BenchConfig) : IO UInt32 := do
  if cfg.repeats == 0 || cfg.number == 0 then
    IO.eprintln "repeat and number must be positive."
    return 1
  let mut results : Array CaseBenchmark := #[]
  for c in cfg.cases do
    let r ← benchmarkCase c cfg.repeats cfg.number cfg.warmup cfg.targetMs
    results := results.push r
    if !cfg.json then
      printCaseBenchmark r
  if cfg.json then
