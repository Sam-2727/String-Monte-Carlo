...
\textbf{Validated} (87/87 tests passing in the automated suite):
\begin{itemize}
...
\item Stable pre-GSO Bose--Fermi ratio scan: no unsummed single-cylinder sector is yet close to the target ratio $1$ on the sampled grid. The closest sampled sector has distance $0.34085$ from $1$, while the largest sampled log-magnitude is already $5.04\times 10^3$. This makes the missing spin-structure/GSO and zero-mode assembly explicit.
\end{itemize}

\textbf{Remaining:}
\begin{enumerate}
\item Derive the genuinely local branch-point-normalized finite-$N$ fermionic interaction-point variable and check whether it reduces to the present canonical endpoint-difference candidate in the three-point vacuum problem.
...
\item Assemble the physical spin-structure/GSO and zero-mode factors on top of the single-cylinder prototype; the unsummed sector scan already shows that pre-GSO oscillator sectors do not cancel by themselves.
