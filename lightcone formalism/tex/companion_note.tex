...
For loop diagrams one includes a $\sigma$-twist: the spectral shift operator $R_N(\varphi)$ multiplies the $k$th Fourier mode by $e^{2\pi\ii\kappa_k\varphi}$, where $\kappa_k = k$ for $0 \le k \le \lfloor(N-1)/2\rfloor$ and $\kappa_k = k - N$ otherwise.
...
\item $v_{IJ}(\Lambda)$: the SO$(8)$-covariant octic polynomial in the surviving fermionic zero-mode combination $\Lambda^a \equiv \sqrt{N_1 N_2/N_3}\,(\theta_{\mathrm{av}}^{(1)a} - \theta_{\mathrm{av}}^{(2)a})$, where $\theta_{\mathrm{av}}^{(r)a} = N_r^{-1}\sum_n \theta_n^{(r)a}$ is the average GS fermion on leg~$r$, and $a = 1, \dots, 8$ is the SO$(8)$ chiral spinor index.
\end{itemize}
...
The tree-level fermionic factor can now be computed directly. In the local $\Lambda$-superfield convention $v^{ij}(\Lambda) \leftrightarrow 16\,\Sigma^j\widetilde\Sigma^i$ from the Dijkgraaf--Motl dictionary, a closed-string graviton with polarization $\epsilon_{ij}$ has Grassmann wavefunction $\Psi_\epsilon(\lambda) = (1/16)\,\epsilon_{ij}\,v^{ji}(\lambda)$. The three-string kinematic measure contains $\delta^8(\lambda_1 + \lambda_2 + \lambda_3)$, which after integration reduces the zero-mode factor to a 16-Grassmann integral:
\begin{equation}
...
lightcone cubic prefactor in the same local convention. In the continuum one has
\(
...
\eqref{eq:ferm-assembled-closed-family-2}. This confirms that the discrete vertex reproduces the correct \emph{tensor structure} of the continuum GS cubic prefactor---selection rules, channel ratios, and $\lambda$-dependence---to machine precision. What it does not yet test is the absolute value of $B_{qq}$ itself: the continuum predicts $B_{qq}\propto \alpha_3^2|q_{\mathrm{rel}}|^2$ (up to the cubic coupling constant), and matching that proportionality coefficient to the known continuum normalization is the remaining open step.
...
\label{eq:ferm-rank1}
\end{equation}
The relative Frobenius error of this rank-one fit is likewise $1.11\times 10^{-4}$, and the maximum normalized-profile deviation after calibration at $\lambda = 2/5$ is $6.97\times 10^{-4}$. So the explicit fermionic contraction, not merely the Weyl proxy, confirms a universal $\lambda$-profile across the unblocked stencil family with residual ambiguity only in the overall normalization. In the automated test suite, all 46 tests pass.
...
\textbf{Validated} (46/46 tests passing in the automated suite):
\begin{itemize}
...
\item Direct structural comparison to the known flat-space GS lightcone cubic benchmark $\,\mathbb{P}^I\mathbb{P}^Jv_{IJ}(\Lambda)\,$ agrees on the same scan with the same $1.60\times 10^{-14}$ maximum error, up to the still-unfixed overall normalization.
...
\item Explicit fermionic Grassmann contraction~\eqref{eq:ferm-channels} in the fixed local $\Lambda$-superfield convention: dilaton and $B$-field channels vanish exactly; graviton channels show the correct transverse-rotation ratios.
\end{itemize}

\textbf{Remaining:}
\begin{enumerate}
\item Fix the overall cubic normalization by matching to the continuum three-graviton and three-tachyon couplings.
\item Implement the twisted propagator $K_B(T, \varphi)$ and assemble the first loop integrand.
\item Spin-structure/GSO bookkeeping for physical superstring loops.
\end{enumerate}

The method reduces higher-loop superstring amplitudes to exact Gaussian sewing plus numerical moduli integration. The tree-level results provide strong evidence that the scheme converges to the correct continuum amplitudes. The standard continuum lightcone construction is taken as known background; the contribution of this work is to show that the discrete-$\sigma$ method reproduces it numerically with controlled finite-$N$ extrapolation.
