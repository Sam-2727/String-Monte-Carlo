...
For loop amplitudes one must distinguish two binary pieces of fermionic
spin-structure data. The first is the spatial boundary condition around the
$\sigma$-cycle,
\[
\nu\in\Bigl\{0,\frac12\Bigr\},
\]
with $\nu=0$ for periodic (Ramond-like) fermions and $\nu=\frac12$ for
antiperiodic (Neveu--Schwarz-like) fermions. The second is the sign
$s\in\{\pm1\}$ entering the coherent-state trace around the Euclidean time
cycle. For fixed $(\nu,s)$, define the shifted finite-$N$ transport matrix
\begin{equation}
U_\nu(T,\varphi)
=
F_\nu^\dagger
...
\bigr)
F_\nu,
\label{eq:ferm-spin-transport}
\end{equation}
where $(F_\nu)_{kn}=N^{-1/2}e^{2\pi\ii(k+\nu)n/N}$ and
\[
\omega_{k+\nu}=\frac{2}{a}\sin\!\Bigl(\frac{\pi|k+\nu|}{N}\Bigr).
\]
The corresponding oscillator-sector one-cylinder factor is
\begin{equation}
Z^{(1)}_{F,\nu,s}(T,\varphi)
\equiv
\det\!\bigl(\mathbf{1}+s\,U_{\nu,\mathrm{osc}}(T,\varphi)\bigr),
\label{eq:ferm-spin-sector}
\end{equation}
where for $\nu=0$ the $k=0$ zero mode is removed from the determinant and
treated separately, while for $\nu=\frac12$ there is no zero mode. In the
notation used later,
\[
\Bigl(\nu,s\Bigr)=\Bigl(\frac12,+1\Bigr)\leftrightarrow \mathrm{NS},\quad
\Bigl(\nu,s\Bigr)=\Bigl(\frac12,-1\Bigr)\leftrightarrow \widetilde{\mathrm{NS}},\quad
\Bigl(\nu,s\Bigr)=\Bigl(0,+1\Bigr)\leftrightarrow \mathrm{R},\quad
\Bigl(\nu,s\Bigr)=\Bigl(0,-1\Bigr)\leftrightarrow \widetilde{\mathrm{R}}.
\]
Thus the notation
\[
Z^{(1)}_{F,\alpha}(T,\varphi),
\qquad
\alpha\in\{\mathrm{NS},\widetilde{\mathrm{NS}},\mathrm{R},\widetilde{\mathrm{R}}\},
\]
is only shorthand for the four choices of $(\nu,s)$ above. A physical closed
string loop amplitude would require \emph{two} independent chiral choices,
say $\alpha_L$ and $\alpha_R$, together with their zero-mode factors and
overall phases:
\begin{equation}
...
\bigl[Z^{(1)}_{F,\alpha_L}(T,\varphi)\bigr]^8
\bigl[Z^{(1)}_{F,\alpha_R}(T,\varphi)\bigr]^8.
\label{eq:ferm-closed-osc}
\end{equation}
The present loop numerics do \emph{not} yet claim to evaluate this full
quantity. What is currently implemented is the restricted trial ratio
\begin{equation}
{\cal R}_{\mathrm{trial}}^{\mathrm{osc}}(T,\varphi;c)
\equiv
\frac{\Bigl(\sum_{\alpha} c_\alpha\,[Z^{(1)}_{F,\alpha}(T,\varphi)]^8\Bigr)^2}
{\bigl(Z_B^{(1)}(T,\varphi)\bigr)^8},
\label{eq:ferm-trial-ratio}
\end{equation}
which assumes the same chiral coefficients on the left and right and omits
the independent zero-mode factors as well as the final coherent-state/Pfaffian
normalization relative to the bosonic determinant. This distinction matters:
the current negative GSO-sign scan rules out ``sign choice only'' as a
solution, but it does \emph{not} yet rule out the fully assembled physical
type-II loop integrand.

\begin{figure}[t]
\centering
\begin{tikzpicture}[scale=0.9, every node/.style={font=\small}]
\node[draw,rounded corners,align=center,text width=4.0cm,inner sep=5pt] (quartet) at (-4.0,0.2)
{\textbf{Explicit one-cylinder data}\\[2pt]
$Z^{(1)}_{F,\alpha}(T,\varphi)$ for\\
$\alpha\in\{\mathrm{NS},\widetilde{\mathrm{NS}},\mathrm{R},\widetilde{\mathrm{R}}\}$};

\node[draw,rounded corners,align=center,text width=4.8cm,inner sep=5pt] (physical) at (2.5,1.45)
{\textbf{Physical loop target}\\[2pt]
$Z_{\mathrm{closed}}^{\mathrm{osc}}(T,\varphi;c^{(L)},c^{(R)})$\\
plus zero modes and normalization/phases};

\node[draw,rounded corners,align=center,text width=4.8cm,inner sep=5pt,fill=gray!10] (trial) at (2.5,-1.15)
{\textbf{Current implemented scan}\\[2pt]
${\cal R}_{\mathrm{trial}}^{\mathrm{osc}}(T,\varphi;c)$\\
same chiral coefficients on both sides,\\
zero modes omitted};

\draw[->,thick] (-1.95,0.5) -- (0.0,1.05);
\draw[->,thick] (-1.95,-0.05) -- (0.0,-0.7);
\end{tikzpicture}
\caption{Loop spin-structure bookkeeping in the companion note. The finite-$N$ cylinder now supplies the full oscillator quartet explicitly, but the current sign scan still probes only the restricted trial ratio~\eqref{eq:ferm-trial-ratio}, not the final physical type-II loop integrand.}
\label{fig:companion-loop-bookkeeping}
\end{figure}
...
This convergence now has a direct analytic target. In the positive-width outgoing convention $\alpha_3=\alpha_1+\alpha_2>0$, the standard continuum lightcone cubic vertex gives
\begin{equation}
...
\label{eq:companion-gamma-cont}
\end{equation}
The helper \texttt{continuum\_tachyon\_benchmark.py} compares the fixed-ratio Richardson extrapolations of the discrete Schur complement against this analytic continuum target on the families $1\!:\!3$, $1\!:\!2$, $3\!:\!5$, $2\!:\!3$, and $1\!:\!1$ with scales $8,16,32,64,128,256$. The observed agreement is
\begin{equation}
...
=5.37\times 10^{-5}.
\label{eq:companion-gamma-cont-error}
\end{equation}
There is also an exact on-shell factor split in the same convention:
\begin{equation}
\log \mu(\alpha_1,\alpha_2,\alpha_3)^2
=
\frac{q_{\mathrm{rel}}^2}{2\gamma_T^{\mathrm{cont}}},
\label{eq:companion-mu-split}
\end{equation}
so the continuum Mandelstam prefactor is controlled by the same Schur complement. The helper \texttt{continuum\_tachyon\_factor\_split.py} verifies both this identity and the discrete Richardson limit on the sampled family set:
\begin{equation}
\max\left|\frac{q_{\mathrm{rel}}^2}{2\gamma_T^{\mathrm{cont}}}-\log\mu^2\right|
=8.88\times 10^{-16},
\qquad
\max\left|\frac{q_{\mathrm{rel}}^2}{2\gamma_T^{(\infty)}}-\log\mu^2\right|
=1.01\times 10^{-6},
\label{eq:companion-mu-split-error}
\end{equation}
with maximum relative error $2.42\times 10^{-7}$ in the second comparison. So the bosonic three-tachyon sector now matches the known continuum Schur-complement kinematics and the full on-shell Mandelstam exponential quantitatively, while the overall cubic normalization remains a separate question encoded in ${\cal C}_{\mathrm{tail}}$.
The remaining scalar prefactor can now also be written explicitly on the same
fixed-ratio families. Defining
\begin{equation}
R_{\mathrm{pref}}(N_1,N_2)
\equiv
\log C_{\mathrm{req}}^{(B)}(N_1,N_2)-\log\mu(\alpha_1,\alpha_2,\alpha_3)^2,
\label{eq:companion-pref-remainder}
\end{equation}
the helper \texttt{tachyon\_prefactor\_remainder.py} removes the universal
fixed-family scale tail and compares the extrapolated remainder to
\begin{equation}
...
{\cal C}_{\mathrm{tail}}
 ...
 - \log\mu(a,b,a+b)^2.
\label{eq:companion-pref-target}
\end{equation}
On the sampled five-family set the agreement is
\begin{equation}
...
=3.48\times 10^{-9}.
\label{eq:companion-pref-target-error}
\end{equation}
So the remaining bosonic normalization problem is now isolated to an explicit
scalar prefactor target, even though the final Mandelstam/HIKKO coupling match
...
\qquad
\mathcal{P}_{I,\mathrm{loc}} = K_{\mathrm{lat}}^I\,\widetilde K_{\mathrm{lat}}^J\,v_{IJ}^{\mathrm{lat}}(\Lambda_{\mathrm{join}}),
\label{eq:vgs}
\end{equation}
where $|V_{\mathrm{kin}}\rangle$ is the exact bosonic-plus-fermionic kinematic overlap, and the superscripts $I, J = 1, \dots, \perpD$ are transverse SO$(\perpD)$ vector indices. Here $\Lambda_{\mathrm{join}}$ denotes the genuinely local interaction-point fermion variable. The reduced overlap-constrained coordinate $\Lambda_{\mathrm{lat}}$ introduced below is only the current three-point ansatz obtained after dropping the explicit local correction $\Xi_{\mathrm{loc}}$. The three factors are:
...
At the continuum level, however, the Dijkgraaf--Motl and
Pankiewicz--Stefanski conventions are now aligned explicitly rather than only
qualitatively. Dijkgraaf--Motl define the local regulated interaction-point
fermion by
\[
\Lambda^a_{\mathrm{DM}}
=
...
\]
and state that it is related to the Brink--Green--Schwarz variable by
\begin{equation}
\Lambda^a_{\mathrm{DM}}
=
\sqrt{\frac{2}{\alpha}}\,
\Lambda^a_{\mathrm{GSB}}.
\label{eq:dm-ps-bridge}
\end{equation}
So a degree-$d$ monomial differs only by the universal factor
$(\alpha/2)^{d/2}$. The helper \texttt{lambda\_convention\_bridge.py} verifies
directly on the imported prefactor tensors that this rescaling removes the
explicit $\alpha$-dependence and reproduces the Dijkgraaf--Motl normalized
coefficients to machine precision. The remaining ambiguity is therefore not a
continuum DM-versus-PS mismatch, but the finite-$N$ lattice realization of the
local branch-point operator itself.
...
The finite-$a$ standpoint of this note is to keep these two representatives distinct and to treat the interaction point as a resolved bond-centered local region, not as a single pre-existing lattice site. The exact cubic sewing is still the simple ordered concatenation of the three boundaries; the nontrivial branch-point geometry is deferred to the local prefactor and any finite-$a$ counterterms. This is the reason the local bosonic and fermionic prefactors are built from one-sided arc data attached to $I_+$ and $I_-$ rather than from a separately inserted midpoint site. One could certainly design a \emph{different} discretization in which the branch point is represented by one special site, but then the overlap would no longer be the bare concatenation map: either one identifies two previously independent endpoint variables or one inserts a new interaction-point degree of freedom, and in both cases the local kinetic terms and prefactors have to be reformulated. So the present scheme should be read as a deliberate finite-$a$ choice, not as a claim that a single-site version is impossible in principle.
...
\end{equation}
For the most general endpoint-linear local candidate
\begin{equation}
\Lambda_{\mathrm{trial}}^a
\equiv
c_+\,\theta_{I_+}^a+c_-\,\theta_{I_-}^a,
\label{eq:theta-local-trial}
\end{equation}
the exact mixed zero-mode piece is
\begin{equation}
\Lambda_{\mathrm{trial}}^a
=
...
\;+\;(\text{nonzero modes}),
\label{eq:theta-local-trial-mixed}
\end{equation}
so imposing zero $\Theta_{\mathrm{cm}}$ contamination and unit
$\Lambda_{\mathrm{lat}}$ coefficient gives
\begin{equation}
c_+ + c_- = 0,
\qquad
c_+\sqrt{\frac{N_2}{N_1N_3}}
-c_-\sqrt{\frac{N_1}{N_2N_3}}
=1,
\label{eq:theta-local-trial-constraints}
\end{equation}
...
\qquad
c_-=-\sqrt{\frac{N_1N_2}{N_3}}.
\label{eq:theta-local-trial-solution}
\end{equation}
So the scaled endpoint difference is the unique endpoint-linear choice with no
center-of-mass contamination and unit reduced coefficient.
The new helper \texttt{local\_endpoint\_phase\_scan.py} strengthens this by
scanning the equal-magnitude phase family
\[
...
\]
normalized so that the reduced $\Lambda_{\mathrm{lat}}$ coefficient is $1$.
Within that family, $\phi=\pi$ is the unique CM-free point and also minimizes
the local two-point scalar. At $(N_1,N_2)=(128,192)$ the naive DM-style
endpoint sum with phase $\phi=\pm\pi/2$ has
$|\Theta_{\mathrm{cm}}|\approx 1.72\times 10^1$, whereas the canonical
anti-phase point has $|\Theta_{\mathrm{cm}}|=0$. So a literal finite-$N$
endpoint sum with relative phase $\pm\ii$ is not equivalent to the canonical
difference and would require further local completion.
...
\end{equation}
and the corresponding trace-dropped $\delta^{IJ}$ benchmark channel vanishes on the same grid. So, for this canonical endpoint-difference local candidate, the benchmark graviton channels already reduce exactly to the old reduced ansatz before any further nonzero-mode averaging, while the genuinely local correction sectors first appear in channels that are absent in the reduced benchmark. The helper \texttt{local\_channel\_catalog.py} extends this from the benchmark set to the full $5^3$ polarization catalog built from $\{\epsilon_\parallel,\epsilon_{23},\epsilon_{24},\epsilon_{\mathrm{dil}},B_{23}\}$. On the same sampled ratio grid and in the trace-dropped $qq$ response, the 125 channels split into only five exact classes: 47 vanish identically, 37 are purely quadratic in $\Xi_{\mathrm{loc}}$, 16 are pure reduced degree-$0$ responses, 16 are purely quartic, and 9 contain both reduced and quartic pieces. For the trace-dropped $\delta^{IJ}$ response, all 125 sampled channels vanish. This makes clear that the benchmark graviton collapse is part of a small exact finite-$N$ classification rather than an isolated special case.
For the same canonical endpoint-difference candidate, the helper \texttt{local\_vacuum\_reduction.py} now performs the first explicit three-point nonzero-mode vacuum contraction. In the product external oscillator vacuum one has
\begin{equation}
...
\label{eq:theta-local-vacuum-two-point}
\end{equation}
so every positive-degree $\Xi_{\mathrm{loc}}$ monomial in the local catalog has zero vacuum expectation because its SO$(8)$ indices are all distinct. Therefore the sampled trace-dropped catalog collapses after contraction to
\begin{equation}
\#(\text{vanishing})=100,
\qquad
\#(\text{reduced only})=25,
...
\label{eq:theta-local-vacuum-collapse}
\end{equation}
so, for the present canonical local candidate and three-point vacuum external states, the explicit nonzero-mode reduction produces no correction beyond the degree-$0$ reduced ansatz.
The new helper \texttt{local\_superstring\_tree\_benchmark.py} then assembles the actual trace-dropped benchmark amplitudes from these vacuum-contracted local responses and the bosonic coefficients $(A_\delta,B_{qq})$. On the sampled family set $1{:}3,1{:}2,3{:}5,2{:}3,1{:}1$ with scales $L=16,32,64,128$ it gives
\begin{equation}
\max_{\mathrm{bench}}\bigl|\mathcal{A}^{\mathrm{loc,vac}}_F-\mathcal{A}^{\mathrm{red}}_F\bigr|=0,
\qquad
\max_{\mathrm{bench}}\bigl|\mathcal{A}^{\mathrm{loc,vac}}_F-\mathcal{A}^{\mathrm{bench}}_{\mathrm{GS}}\bigr|<10^{-14},
\label{eq:theta-local-tree-benchmark}
\end{equation}
so, for this current canonical local candidate and three-point vacuum external states, the local construction reproduces the same benchmark graviton, dilaton, and antisymmetric-channel answers as the reduced calculation, but now by explicit local-vacuum reduction rather than by direct insertion of the reduced ansatz.
The new helper \texttt{local\_arc\_catalog\_scan.py} sharpens the same point at
the level of the full sampled vacuum-contracted catalog. For the simple
one-sided arc family
\[
\Lambda_{\mathrm{trial}}
=
\Lambda_{\mathrm{join}}
+c_{\nabla,+}\,\nabla_+\theta
+c_{\nabla,-}\,\nabla_-\theta
\]
with sampled coefficients
\[
(c_{\nabla,+},c_{\nabla,-})
\in
\left\{
...
\right\},
\]
the entire trace-dropped vacuum-reduced catalog remains unchanged on the same
sampled $\lambda$ grid:
\begin{equation}
...
=
125\ \text{vanishing}.
\label{eq:theta-local-arc-catalog}
\end{equation}
So the present three-point vacuum problem fixes the endpoint-linear part of the
local fermion but does not yet distinguish these simple pure-local arc
admixtures; the remaining uncertainty lives in the deeper local completion and
its branch-point normalization, not in the reduced zero-mode coordinate itself.
The same conclusion extends to the larger sampled nearest-neighbor family once
one combines this arc scan with the degree classification of the local catalog.
On the sampled grid, the uncontracted trace-dropped catalog contains only
$\Xi_{\mathrm{loc}}$ degrees $0,2,4$, so after the diagonal vacuum reduction
\[
\langle \Xi_{\mathrm{loc}}^a \Xi_{\mathrm{loc}}^b\rangle_{\mathrm{vac}}
=
C_\Xi\,\delta^{ab}
\]
every contracted channel is at most quadratic in the single scalar $C_\Xi$.
The arc-family scan already samples five distinct values of $C_\Xi$ and gives
zero discrepancy from the reduced catalog at all five. Therefore the contracted
sampled catalog is identically independent of $C_\Xi$ for any linear local
candidate with the same mixed zero-mode data $(\Theta_{\mathrm{cm}},
\Lambda_{\mathrm{lat}})=(0,1)$ and a spinor-diagonal vacuum reduction. In
particular, for the broader nearest-neighbor completion
\[
\Lambda_{\mathrm{nn}}
=
\Lambda_{\mathrm{join}}
...
\]
the new helper \texttt{local\_nearest\_neighbor\_family\_scan.py} again finds
zero benchmark and catalog discrepancy on a representative sampled coefficient
set. So the present three-point vacuum problem fixes the endpoint-linear core
uniquely but still does not determine the broader join-supported linear
completion.
...
\label{eq:ferm-rank1}
\end{equation}
The relative Frobenius error of this rank-one fit is likewise $1.11\times 10^{-4}$, and the maximum normalized-profile deviation after calibration at $\lambda = 2/5$ is $6.97\times 10^{-4}$. So the explicit fermionic contraction, not merely the Weyl proxy, confirms a universal $\lambda$-profile across the unblocked stencil family with residual ambiguity only in the overall normalization. The current machine-readable regression checkpoint is the aggregated suite in \texttt{numerical\_suite.py}.
...
\textbf{Validated} (see the current aggregated checkpoint in \texttt{numerical\_suite.py}):
\begin{itemize}
\item Overlap algebra to $10^{-15}$.
\item Bosonic three-tachyon factorization at $\perpD = 24$ with $10^5$ separation ratio.
\item Continuum bosonic Schur benchmark: the Richardson-extrapolated fixed-ratio $\gamma_T$ values agree with the analytic continuum target~\eqref{eq:companion-gamma-cont} with maximum absolute error $1.94\times 10^{-5}$ and maximum relative error $5.37\times 10^{-5}$ on the sampled family set.
\item Continuum bosonic factor split: the on-shell identity~\eqref{eq:companion-mu-split} holds to $8.88\times 10^{-16}$, and the discrete Richardson limit matches $\log\mu(\alpha)^2$ with maximum absolute error $1.01\times 10^{-6}$ and maximum relative error $2.42\times 10^{-7}$ on the sampled family set.
\item Bosonic tachyon prefactor remainder: after removing the continuum $\mu(\alpha)^2$ exponential and the universal fixed-family scale tail, the extrapolated scalar remainder matches the explicit target~\eqref{eq:companion-pref-target} with maximum absolute error $6.36\times 10^{-8}$ and maximum relative error $3.48\times 10^{-9}$ on the sampled family set.
\item $2T+1M$ tensor: $A_{\mathrm{tr}} \approx 0$, $B_{\mathrm{rel}} \to 1.920$ with continuum error bar $\pm 7 \times 10^{-4}$.
\item Bosonic normalization structure: on the $4\le N_1,N_2\le 60$, $N_3\le 120$ grid the invariant three-leg tail gives ${\cal C}_{\mathrm{tail}}=-22.496054835$ with RMS residual $2.48\times 10^{-7}$, while the fixed incoming/outgoing nonlinear tails $(7,\pi,\pi^2/72)$ and $(-5,-\pi,\pi^2/72)$ leave only linear-plus-constant remainders with RMS residuals $1.61\times 10^{-12}$ and $2.91\times 10^{-11}$.
...
\item Continuum DM/PS fermion-variable convention bridge: the degree-rescaled prefactor tensors satisfy \eqref{eq:dm-ps-bridge} numerically, becoming $\alpha$-independent and reproducing the Dijkgraaf--Motl normalized coefficients to machine precision.
...
\item Endpoint-linear local fermion uniqueness: among linear combinations of $\theta_{I_+}$ and $\theta_{I_-}$, the scaled endpoint difference is the unique finite-$N$ choice with zero $\Theta_{\mathrm{cm}}$ contamination and unit $\Lambda_{\mathrm{lat}}$ coefficient.
\item Endpoint-phase scan: after unit-$\Lambda_{\mathrm{lat}}$ normalization, the canonical anti-phase endpoint difference is the unique CM-free equal-magnitude endpoint choice and also minimizes the local two-point scalar, whereas the naive $\theta_{I_+}+\ii\theta_{I_-}$ and $\theta_{I_+}-\ii\theta_{I_-}$ sums carry large center-of-mass contamination.
...
\item Full local channel catalog: on the sampled ratio grid the trace-dropped $qq$ response over the full $5^3$ polarization basis splits into 47 vanishing channels, 37 purely quadratic local channels, 16 pure reduced channels, 16 pure quartic local channels, and 9 mixed reduced-plus-quartic channels; the trace-dropped $\delta^{IJ}$ response vanishes for all 125 sampled channels.
\item Explicit local nonzero-mode vacuum reduction for the same canonical endpoint-difference candidate: after contraction, the sampled trace-dropped local catalog collapses to 100 vanishing channels and 25 pure reduced channels, with the benchmark dilaton quartic sector vanishing as in \eqref{eq:theta-local-vacuum-collapse}.
\item Local three-point benchmark assembled from the vacuum-contracted local responses: on the sampled family set, the current canonical local candidate reproduces the same benchmark graviton, dilaton, and antisymmetric-channel amplitudes as the reduced calculation, with exact local-vs-reduced agreement and maximum benchmark mismatch below $10^{-14}$ as in \eqref{eq:theta-local-tree-benchmark}.
\item Broader nearest-neighbor local family: the full sampled three-point vacuum catalog stays unchanged even after extending the canonical endpoint difference by the four join-supported nearest-neighbor rows on the two incoming legs. So, within this larger linear family, the present three-point vacuum problem still fixes only the endpoint-linear core and not the full local completion.
\item Interaction-region geometry: the current finite-$a$ scheme is kept as an exact pairwise sewing together with a resolved bond-centered local interaction region supported on $(I_+,I_-)$ and adjacent one-sided arc data, rather than being reformulated around a single distinguished lattice site.
...
\item Explicit fermionic oscillator spin structures on one cylinder: the new periodic/antiperiodic sector helper reproduces the older periodic factors to $2.45\times10^{-15}$, matches the antiperiodic direct determinants to their shifted closed mode products to $2.56\times10^{-15}$, and makes the antiperiodic mass gap explicit through a strictly positive minimum sampled oscillator frequency.
\item Stable pre-GSO Bose--Fermi ratio scan: no unsummed single-cylinder sector is yet close to the target ratio $1$ on the sampled grid. The closest sampled sector has distance $0.34085$ from $1$, while the largest sampled log-magnitude is already $5.04\times 10^3$. This is an oscillator-sector statement only; it intentionally omits both the zero-mode factors and the final GSO/spin-structure combination.
\item Oscillator spin-structure sign scan: after promoting the loop fermions to the full $(\mathrm{NS},\widetilde{\mathrm{NS}},\mathrm{R},\widetilde{\mathrm{R}})$ oscillator quartet, the trial closed-string ratio built from the same chiral sign pattern on the left- and right-moving sides is still far from $1$. Even the best sampled sign pattern has closest distance $0.304799$ and worst-case distance $6.84\times 10^2$; the standard $\mathrm{NS}-\widetilde{\mathrm{NS}}-\mathrm{R}+\widetilde{\mathrm{R}}$ pattern is no better. So the remaining loop problem is not just missing sign bookkeeping, but the zero-mode and normalization assembly.
\end{itemize}

\textbf{Remaining:}
\begin{enumerate}
\item Derive the genuinely local branch-point-normalized finite-$N$ fermionic interaction-point variable and determine whether any local operators beyond the present linear nearest-neighbor family survive in the continuum three-point amplitude.
\item Fix the overall cubic normalization by matching the remaining bosonic and superstring overall coefficients to the continuum three-tachyon and three-graviton couplings. The bosonic Schur-complement kinematics are now already matched, so the unresolved part is the overall normalization rather than the reduced Gaussian structure.
\item Assemble the physical spin-structure/GSO and zero-mode factors on top of the single-cylinder prototype; the new oscillator spin-structure scan shows that even after adding periodic/antiperiodic sectors, sign choices alone do not produce the expected cancellation.
...
The method reduces higher-loop superstring amplitudes to exact Gaussian sewing plus numerical moduli integration. On the bosonic tree side, the current evidence is now stronger than a qualitative convergence claim: the factorization test, the critical-dimension signal, the direct continuum Schur-complement match, and the $2T+1M$ tensor check all point in the same direction, leaving the overall cubic normalization as the main open bosonic amplitude-level task. On the superstring side, the current tree-level results should still be read more cautiously: they now show that, for the present canonical endpoint-difference local candidate, the explicit three-point nonzero-mode vacuum reduction adds no correction beyond the reduced ansatz on the sampled local catalog, but the genuinely local branch-point-normalized finite-$N$ fermionic interaction-point operator still needs to be derived before the superstring cubic vertex can be regarded as established.
