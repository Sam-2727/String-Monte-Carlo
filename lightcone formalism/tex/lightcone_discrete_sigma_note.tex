...
\end{cases}
\end{equation}
so that $\kappa_{N-k}=-\kappa_k$ except for the self-conjugate Nyquist mode when $N$ is even. Then
\begin{equation}
...
\end{equation}
which matches the number of nonzero complex mode labels on the three legs. The actual number of independent real oscillators is obtained only after imposing the involution $a_{N_r-m}^{(r)}=(a_m^{(r)})^\dagger$, or equivalently after passing to an independent half-spectrum or real sine/cosine basis. Let $A_{\mathrm{phys}}$ denote such a basis, and let $\mathbb M_{\mathrm{phys}}$, $\mathbb N_{\mathrm{phys}}$, $J_{0,\mathrm{phys}}$ be the corresponding reduced matrices. Since each $(\mu_r\Omega_r)^{\pm1/2}$ is invertible on the nonzero-mode sector, nondegeneracy of $\mathbb M_{\mathrm{phys}}$ reduces to linear independence of the exact overlap constraints on the physical oscillator space. This is a finite-dimensional statement that can and should be checked numerically at each lattice resolution $N$. Assuming $\mathbb M_{\mathrm{phys}}$ is nondegenerate, the exact solution is a displaced squeezed state
\begin{equation}
|V_3^{(B)}\rangle =
{\cal C}_3^{(B)}
\delta^{(\perpD)}(p_\perp^{(1)}+p_\perp^{(2)}-p_\perp^{(3)})
\exp\!\left[
 -\frac12 A_{\mathrm{phys}}^{\dagger T}\,S^{(B)}\,A_{\mathrm{phys}}^\dagger
 -A_{\mathrm{phys}}^{\dagger T}\Lambda^{(B)}(y)
 -\frac12\Phi^{(B)}(y)
\right]|0\rangle,
\end{equation}
with
...
\delta^{(\perpD)}(p_\perp^{(1)}+p_\perp^{(2)}-p_\perp^{(3)})},
\end{equation}
so $\Phi^{(B)}(y)$ is a quadratic form in the relative center-of-mass displacement $y$ determined uniquely once the exact overlap state is normalized. The matrix $S^{(B)}$ is the oscillator squeeze matrix in the chosen physical basis. If one lifts it back to the redundant full-spectrum notation, then with $\mathcal R=\mathrm{diag}(\mathcal R_1,\mathcal R_2,\mathcal R_3)$ and $(\mathcal R_r)_{mn}=\delta_{m,N_r-n}$ one recovers the reality conditions
\begin{equation}
...
\end{pmatrix}.
\end{equation}
This is the fermionic analogue of the bosonic overlap system, without the frequency factors $(\mu\Omega)^{\pm1/2}$ because the GS fermions are first order rather than second order in worldsheet time. Let $B_{\mathrm{phys}}$ denote an independent fermionic oscillator basis obtained from the full complexified nonzero modes by imposing the involution or, equivalently, by passing to a real half-spectrum basis. Let $\mathbb M_{F,\mathrm{phys}}$, $\mathbb N_{F,\mathrm{phys}}$ be the corresponding reduced overlap matrices. Then the kinematic overlap state solves linear constraints of the form
\begin{equation}
