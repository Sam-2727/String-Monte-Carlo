...
\ie
\Psi[e_{ij},p^i; X^i] \propto e_{ij} X_1^i X_{-1}^j \exp \left[ -\frac{1}{\A'} \sum_m m X_m^i X_{-m}^i + i p^i X^i_0 \right],
\fe
...
&\qquad \left [ \delta^{ij} v_1 \cdot \wt{M}^{-1} \cdot v_{-1} - \pi \A' (v_1 \cdot \wt{M}^{-1} \cdot \wt{J}^i) (v_{-1} \cdot \wt{M}^{-1} \cdot \wt{J}^j)  \right].
\fe
...
\ie
\lim_{L\to \infty} \left|\frac{1}{L^2} v_1 \cdot \wt{M}^{-1} \cdot \chi_1 \right| = (\text{const}) ~  \frac{(x (1-x))^2}{x^{2x} (1-x)^{2(1-x)}},
\fe
precisely canceling the extra factors contributed by (\ref{eq:edotJ}) and (\ref{eq:extraScaling})! Thus besides the expected transverse momentum dependence $e_{ij} (p_1^i - p_2^i) (p_1^j - p_2^j)$, the only other dependence on $p_1$ and $p_2$ is exactly the same $\frac{1}{\sqrt{x(1-x)}}$ as in the 3 tachyon case. \fixme{make sure the trace term works as well} 
\end{document}
