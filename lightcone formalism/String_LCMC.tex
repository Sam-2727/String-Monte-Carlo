...
\ie
\propto \int \left[ \prod_{k=1}^L X^i(k) \right] \exp \left[ - \frac{1}{2 \pi \A'}  (X^i)^T \cdot M  \cdot X^i +i J^i \cdot X^i \right]e^{i (p + p_1 + p_2) \cdot X_0 }
\fe
with
\ie
...
%We note that the eigenvalues $\lambda_s$ of $M$, with $s=1,...,L$ are close to $\lambda_s = 2 \cos(\frac{\pi (s-1)}{2(L-1)} )$. 
And,
\ie
...
\ie
\propto \delta^{(d-2)}(p + p_1 + p_2)\,  ({\det}' M )^{-\frac{d-2}{2} } \, \exp(-\frac{\pi \A'}{2} J^{i,T} \cdot {M}^{+} \cdot J^i ),
\fe
...
\ie
\propto \delta^{(d-2)}(p + p_1 + p_2)\,  (\det \wt{M} )^{-\frac{d-2}{2} } \, \exp(-\frac{\pi \A'}{2} \wt{J}^{i,T} \cdot \wt{M}^{-1} \cdot \wt{J}^i ).
\fe
We note that for this choice of gauge, $\det \wt{M} = L \,  {\det}' M$. The ratio $\frac{\det \wt{M}(L^{(1)}, L^{(2)}) }{\det \wt{M}(L^{(1)}{}', L^{(2)}{}')}$ converges quickly, but depends on $(L^{(1)}, L^{(2)})$.  Meanwhile, 
\ie
...
\fe
Putting everything together, the 3-pt function becomes (assuming on-shell momenta and using conservation of $p^+$ and $p^i$:
\ie
\propto ~~ &\delta_{(p^+ - p_1^+ - p_2^+),0} \delta^{(d-2)}(p^i + p_1^i + p_2^i) e^{- \pi \A' \frac{(p^+)^3}{p_1^+ p_2^+} f(p_1^+, p_2^+, L) (p_1^- +p_2^- - p^-)} \\
& ~~ \left(\frac{\det \wt{M}(p_1^+,p_2^+,L) }{L} \right)^{-\frac{d-2}{2} }  \exp\left(-\pi \frac{(p^+)^2}{(p_1^+ p_2^+)^2} f(p_1^+, p_2^+, L) ((p^+)^2 + (p_1^+)^2 + (p_2^+)^2)  \right)
\fe
Note that $\delta_{(p^+ - p_1^+ - p_2^+),0}$ is imposed at the level of the discretization scheme. A key test is that we should recover full Lorentz invariance \fixme{of precisely what expression?} for $d=26$ as $L \to \infty$. Conservation of energy should set the first exponential to 1. Therefore we might expect for $d=26$ that the expressions on the second line evaluate to a constant (for fixed $L$ and varying $p_1^+, \, p_2^+$) \fixme{but numerically this doesn't seem to work yet}.
