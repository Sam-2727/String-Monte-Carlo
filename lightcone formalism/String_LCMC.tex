...
\ie
\int \prod_{i=1}^{d-2}\left[ \prod_{k=1}^L dX^i(k) \right] \Psi[p^i; X^i] \Psi^{\text{c}}[q^i; X^i] = (2\pi)^{d-2} \delta^{(d-2)}(p^i+q^i)
\fe
with $\Psi^{\text{c}}$ the conjugate wavefunctional, where $X^i_n \leftrightarrow X^i_{-n}$. We note that in the case that $q^i = - p^i$, the nonzero modes just give a Gaussian integral of $\exp(- \frac{1}{2\pi \A'} (X^i)^T \cdot M' \cdot X^i )$ where 
\ie
