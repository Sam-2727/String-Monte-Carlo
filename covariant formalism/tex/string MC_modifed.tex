...
\end{align}
with similar contributions from the NS/R and R/NS sectors. The R/R sector should evaluate to zero without an insertion of the Ramond sector zero mode. The guess is that $|\det D|^2$ is equivalent to $|\tfrac{\theta_3(0,\tau)}{\eta(\tau)}|e^{1/24 \log|b_i|}$, where $b_i$ is as described in the ``Weyl anomaly'' section, with both $b_i$ empirically having the same magnitude but different phases.
