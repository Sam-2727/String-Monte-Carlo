...
\ie
\Psi_{\partial X \bar\partial X}[X(\sigma)] \propto X_1 X_{-1} \exp \left(-\frac{1}{\A'} \sum_{m\geq 1} mX_m X_{-m}\right).
\fe
...
|\widehat f(0)|^{-2}  \frac{\langle  :\partial X \bar \partial X:(0) \rangle_{D^2(t_1,t_2)}}{\langle  1 \rangle_{D^2(t_1,t_2)}} \sim
\frac{1}{2} |\widehat f(0)|^{-2} \tr( \wt{C} \wt{A}^{-1}),
\fe
where the determinants, and any Weyl anomaly factors should cancel between the numerator and denominator of the RHS calculated in the discretization scheme. Finally, from Polchinski (7.2.16) but with $\partial_{w'} \to \partial_{\bar{w}'}$, we find that \fixme{am I missing terms here from e.g. $\partial_w \vartheta_1(\bar{w}|\tau)|_{w\to 0}$?} 
\ie
...
\ie
\label{trCAexpect}
\tr(\wt{C} \wt{A}^{-1}) = \frac{|\widehat f(0)|^2}{2\tau_2},
\fe
which is modular invariant. In the above, the extra factor of $4\pi^2$ comes from the fact that Polchinski's torus has periods $2\pi$, $2\pi \tau$ while our torus has periods $1$, $\tau$. We also make use of the fact that in our Mathematica implementation the cnvention is $\A' = \frac{1}{\pi}$.

\fixme{Reconcile with the above:}\\
However, we find that, to good numerical accuracy,
\ie
\label{trCAactual}
...
\fe
the shift $v_k$ is then orthogonal to the zero mode, and hence the zero mode can be factored out, giving a factor of $2\pi R$. We can factor out the zero mode by simply setting $X(L/2)$ to 0, and we will denote the restricted matrices (or vectors) on the subspace spanned by $X(1),...,X(\frac{L}{2}-1)$ by a prime. After performing the Gaussian integral in this subspace, the partition function is given by:
\ie
...
Let $z=e^{-iw}$ with the unit disc parameterized by $|z|<1$ or ${\rm Im}(w)>0$. The holomorphic 1-form on the torus can be expressed in the disc coordinate as
\ie\label{fzoneform}
f(z) dz = \sum_{n=1}^\infty a_n z^{n-1} dz = -i \sum_{n=1}^\infty a_n e^{-inw} dw.
\fe
We can normalize with $a_1=1$, and solve $a_2,\cdots, a_L$ numerically by imposing
\ie{}
...
\fe
The relative sign is due to flipping the orientation on $dw$ in the identification. Evaluating the periods of $f(z) dz$ along $\A$ and $\B$ cycle then determines $\tau$ in terms of $\ell_1, \ell_2, \ell_3$.
...
\fe
Here the cutoff is assumed to be $|u-u_i|>\delta$. But should we impose the cutoff on $z$ instead? Near $u=u_i$, we have $z-z_i \approx {2\over 3 b_i} (u-u_i)^{3\over 2}$, and $f(z) \approx b_i ({3b_i\over 2})^{-{1\over 3}} (z-z_i)^{-{1\over 3}}$. If we impose the cutoff in $z$, which would seem to be the more compatible with the discretization scheme on the disc, the moduli dependent part of the Weyl anomaly would be given by
\ie
...
\ie
\Psi_1[c] \propto \prod_{m=2}^{L\over 2} \sum_{k=1}^L c(k) e^{2\pi i {m k\over L}}.
\fe
Next, we identify pairs of $c(k)$'s according to 
\ie\label{pairthreec}
& c(\tfrac{L}{2} + \ell_1+1 - n) = - c(n),~~~~ 1\leq n \leq \ell_1,
\\
& c(\tfrac{L}{2} + 2 \ell_1 + \ell_2 +1 - n) = - c(n),~~~~ \ell_1+1 \leq n \leq \ell_1 + \ell_2,
\\
& c(L+\ell_1 + \ell_2+1 - n) = - c(n),~~~~ \ell_1+\ell_2+1 \leq n \leq \tfrac{L}{2},
\fe
...
\ie{}
& B_{mn} = e^{2\pi i {mn\over L}} - e^{2\pi i {m\over L} (\tfrac{L}{2} + \ell_1+1 - n) },~~~~ 1\leq n \leq \ell_1,
\\
& B_{mn} =  e^{2\pi i {mn\over L}} - e^{2\pi i {m\over L} (\tfrac{L}{2} + 2 \ell_1 + \ell_2 +1 - n)},~~~~ \ell_1+1 \leq n \leq \ell_1 + \ell_2,
\\
& B_{mn} =  e^{2\pi i {mn\over L}} - e^{2\pi i {m\over L} (L+\ell_1 + \ell_2+1 - n) },~~~~ \ell_1+\ell_2+1 \leq n \leq \tfrac{L}{2}.
\fe
...
If we include $bc$ system with 26 free bosons $X^\mu$, so that the Weyl anomalies cancel, one might guess the disc frame correlator to be
\ie
\label{eq:79}
...
\fe
However, the RHS is uncertain, and it is not giving the correct result, so is probably wrong to some extent as a guess. All that can be said is that the answer is modular invariant, since the numerical answer is modular invariant. but we don't have a particular good guess of why. See the attached python files for how this is computed currently. Various things that could be wrong: maybe we have the wrong numerical guess for the b-c determinant, or the wrong analytic formula because we aren't able to guess the position of the b ghosts correctly, or the prefactor that the b-ghost gives near its position is not calculated correctly (perhaps it includes some factor of $b_i$, defined above as the coefficient of the holomorphic one-form near poles).

There are various functions that put together the multiple ingredients present on the RHS of equation \ref{eq:79}. I've tested various combinations of these ingredients in python, using the help of claude code, but none have resulted in an exact match. Much of testing3.ipynb is not useful to our current objectives as it is testing the free fermion (which is a separate problem that we'll get to later), but the final two lines in testing3.ipynb show that, to very high precision, the matter part matches the analytic guess exactly (this is including the Weyl anomaly factor that must be included). 

The goal then is to find the correct guess for \ref{eq:79}, but to do so, it is first important to understand the logic behind the current guess, and also to understand the code that produces that guess and demonstrates that it does not produce the numerical computation, as is computed in the LHS of \ref{eq:79}, as described below.
...
This is observed empirically to be modular invariant.
...
\ie{}
&\psi(\tfrac{L}{2}+\ell_1+1-n)=\pm i e^{\frac{\pi i}{L}(\tfrac{L}{2}+\ell_1+1-2n)}\psi(n),~~~~ 1\leq n \leq \ell_1\\
&\psi(\tfrac{L}{2}+2\ell_1+1-n)=\pm i e^{\frac{\pi i}{L}(\tfrac{L}{2}+2\ell_1+\ell_2+1-2n)}\psi(n),~~~~ \ell_1+1\leq n\leq \ell_1+\ell_2\\
&\psi(L+\ell_1+\ell_2+1-n)=\pm i e^{\frac{\pi i }{L}(L+\ell_1+\ell_2+1-2n)}\psi(n),~~~~ \ell_1+\ell_2\leq n \leq \frac{L}{2}
\fe

with
\ie
\psi_{m+\tfrac{1}{2}}&=\sum_k e^{\frac{2\pi i }{L}(m+\tfrac{1}{2})k}\psi(k)
\fe

and consequently
\ie
\Psi_1[\psi_r]\propto \prod_{r=0}^{\tfrac{L}{2}-1}\sum_{k=1}^L \psi(k) e^{\tfrac{2\pi i}{L}(r+ \tfrac{1}{2})k}
\fe

Then the matrix $D$ is constructed as
\begin{align}
\text{fill this in later}
\end{align}

The NS-NS sector amplitude on the flat torus is
\begin{align}
\sqrt{\frac{\theta_3(0,\tau)}{\eta(\tau)}}
\end{align}

with similar contributions from the NS/R and R/NS sectors. The R/R sector should evaluate to zero without an insertion of the Ramond sector zero mode.
