...
\ie
|\det B| ({\det}' A)^{-13} \propto \tau_2^{-13} |\eta(\tau)|^{-48} |\widehat{f} (0)|^{-2} \left|\frac{1}{3} \sum_{i=1}^3 \left( \lim_{z\to z_i} (1-\frac{z}{z_i})^{2/3} (\widehat{f}(z))^2\right)\right|^2 .
\fe
In other words, $|\det B|$ calculates the $bc \wt{b} \wt{c}$ correlator on the disk with $c \wt{c}$ inserted at the origin, and $b$ and $\wt{b}$ inserted at the Strebel vertices, in the form $\lim_{z\to z_i} (1-\frac{z}{z_i})^{2/3} b(z)$, such that when one considers the transformation to $b(u)$ in the torus coordinate, $ (1-\frac{z}{z_i})^{2/3}$ cancels the divergence of the factors of $f(z)$ that are picked up. Note that in the special case where one of the $\ell_i = 0$, the power must be adjusted to $ (1-\frac{z}{z_i})$.
