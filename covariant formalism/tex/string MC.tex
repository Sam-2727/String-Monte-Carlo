...
Conduct conformal transformation to the disc coordinate, we have
\begin{equation}
\langle :\partial X \bar \partial X:(0) \rangle_{D^2,R} = \langle :\partial X \partial X:(0) \rangle_{T^2,R} -\frac{\alpha'}{12} f''(0),
\end{equation}
We should be able to verify this relation by the discretized computation on the disk.
...
\fe
where $\text{sgn}(i)$ is defined such that $\text{sgn}(i)=1$ if $i<p(i)$ and $-1$ if $i>p(i)$. Among the 6 constraints, only 5 of them are independent. This reduces the independent number of shifts from 9 to 4, which corresponds to the holonomies around the 4 non-contractible 1-cycles. 
