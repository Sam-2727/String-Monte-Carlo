...
\ie
\label{eq:fullGenusOne}
|\det B| ({\det}' A)^{-13} \propto \tau_2^{-13} |\eta(\tau)|^{-48} \frac{1}{|\kappa|^2} \left| \nu \right|^4 ,
\fe
for $\nu$ the average $\nu_i$. It is worth emphasizing that the factor of $\kappa$ in the above formula comes from the insertion of $\hat{f}(z)^2$ from the transformation of $c\tilde{c}$. Although the exact logic can't be pinned down, it seems that the appearance of $|\det B|$ vs. $|\det B|^2$ comes from an identification of holomorphic and anti-holomorphic ghosts at the boundary of the disc.
...
\begin{align}
\frac{1}{(\det \im \Omega)^{13}}\frac{1}{|\chi_{10}(\Omega)|^2}C(\nu)
\end{align}
where $C(\nu)$ is some generalization of the factor $|\nu|^4$ in equation \ref{eq:fullGenusOne}, but now for some suitable basis of holomorphic two-forms on the genus-2 surface (this is just a guess, it hasn't been verified if this is actually the case). One would guess that the factor of $\kappa$ is not present due to the absence of $c$-ghosts. It is also unclear if one has the same holomorphic factorization, where the full anti-holomorphic plus holomorphic b-c system is computed from $|\det B|$ instead of $|\det B|^2$. One sign that there might be complications is that in the genus 1 case, the phase of $\det B$ is independent of moduli, while in the genus 2 case, as far as I can tell, the phase of $\det B$ is not independent of moduli (but double check this).
...
\ie
\frac{\exp(c(\tau)-S_L(\tau))}{Z_\text{comp}^{(1)}(\tau)} = \text{ Const. }
\label{compactWeyltest}
\fe
where $S_L(\tau)$ is given by \eqref{Weylfactor}.
